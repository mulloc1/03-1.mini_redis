...
- `expire`, `ttl` 추가 + 힙 push.
- lazy 만료(접근 시 + 매 명령 후 1회 `_expire_due`).
- SET 덮어쓰기 시 TTL 초기화.
- 테스트: `FakeClock` 으로 시간 이동, stale heap 엔트리 폐기, `EXPIRE 0`/`EXPIRE -1` 즉시 만료, TTL 반환값 3종(`-2`/`-1`/`N`).
- 커밋: `feat: add ttl support with min heap and lazy expiry`
...
- 커밋: `feat: wire repl cli with redis-style output`
